...
import { analyzeCrop } from '../../services/api/cropHealth';
...
export default function CropHealthScreen() {
  ...
  const handleImageSelected = async (uri) => {
    ...
    try {
      setLoading(true);
      const analysisResult = await analyzeCrop(uri);
  ...
  const handleRetry = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const analysisResult = await analyzeCrop(selectedImage);
      setResult(analysisResult);
    } catch (err) {
      setError(err.message || 'Analysis failed. Please try again.');
    }
  ...
  if (result) {
    return (
      <View style={styles.container}>
        <ResultCard result={result} type="crop" />
